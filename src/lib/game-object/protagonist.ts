...
export default class Protagonist extends FirstPerson {
    createSprite(): Sprite {
        ...
        sprite.animations.add('up', [12, 13, 14], this.animationSpeed);
        sprite.animations.add('down', [0, 1, 2], this.animationSpeed);
        sprite.animations.add('left', [6, 7, 8], this.animationSpeed);
        sprite.animations.add('right', [6, 7, 8], this.animationSpeed);
        sprite.animations.add('idle.up', [13]);
        sprite.animations.add('idle.down', [1]);
        sprite.animations.add('idle.left', [7]);
    ...
    preload(): void {
        super.preload();
        this.game.load.spritesheet('torchic.walk', '/assets/pokemon/torchic/walk.png', 16, 24);
    }
}